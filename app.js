...
mongoose.connect("mongodb+srv://admin-vyhuynh:Test123@cluster0.vwjgb.mongodb.net/BogDB");
...
app.get("/", function(req, res) {
    Post.find({}, function(err, foundPosts) {
        if (!err) {
            if (foundPosts) {
                res.render("home", { pageTitle: "Home", homeContent: homeStartingContent, postContent: foundPosts });
            }
        }
    });
...
app.get("/posts/:postId", function(req, res) {
    ...
    Post.findOne({ _id: postId }, function(err, foundPost) {
        if (!err) {
            ...
            res.render("post", { postTitle: "There is an error at the moment, please try again later", postContent: "" });
        }
